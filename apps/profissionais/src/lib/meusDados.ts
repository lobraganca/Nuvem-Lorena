...
export async function baixarMeusDados(userId: string, email: string | undefined): Promise<void> {
  ...
  const conteudo = {
    ...
    observacao:
      "Este arquivo contém os dados pessoais guardados pelo Ei Emprego sobre esta conta. Registros técnicos de acesso, exigidos pelo Marco Civil da Internet, não constam aqui e são guardados por seis meses.",
  };
